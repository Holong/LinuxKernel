...
// zone : contig_page_data.node_zones[ZONE_NORMAL], order : 5, migratetype : 2
// page : 0x20000을 담당하는 struct page
// zone : contig_page_data.node_zones[ZONE_NORMAL], order : 0, migratetype : 2
		...
		int migratetype)
{
	...
	while (order < MAX_ORDER-1) {
		...
		// page : 0x20000을 담당하는 struct page가 됨
		// 즉 합쳐진 공간의 선두 page가 됨
		page = page + (combined_idx - page_idx);
		page_idx = combined_idx;
		// page_idx : 0
		order++;
		// order는 증가
		// 증가시킨 뒤 다시 수행해 상위 buddy 합치기가 가능한 지 확인
	}
	// page: 0x20000 (pfn), order: 5
	set_page_order(page, order);
	// page->private: 5
	// page->_mapcount: -128로 설정
	// 현재 페이지의 buddy order와 free 상태인 것을 표시함
	// 여기까지 오면 상위 합치기가 불가능한 상태임
 ...
 */
// zone : node_zones[ZONE_NORMAL], count : 1, pcp : boot_pageset.pcp
					...
					struct per_cpu_pages *pcp)
{
	...
	while (to_free) {
		...
		do {
			...
			// migratetype : 0, MIGRATE_PCPTYPES : 3
			if (++migratetype == MIGRATE_PCPTYPES)
				migratetype = 0;
			// migratetype : 1

			list = &pcp->lists[migratetype];
			// list : boot_pageset.pcp.lists[1]
		} while (list_empty(list));
		// list : boot_pageset.pcp.lists[2]가 됨
		// lists[1]에는 struct page가 아무 것도 연결되어 있지 않음
		// lists[2]에 struct page가 연결되어 있기 때문에 루프 종료
		// batch_free : 2, migratetype : 2
		...
		do {
			...
			// page : 이전에 lists[2]에 연결해둔 struct page의 주소가 저장됨
			// 	  0x20000을 관리하는 struct page의 주소가 됨

			/* must delete as __free_one_page list manipulates */
			list_del(&page->lru);
			// boot_pageset.pcp.lists[2]에서 struct page를 빼냄

			mt = get_freepage_migratetype(page);
			// page->index 값을 빼옴
			// 현재 page의 migratetype 값을 가져오게 됨

			/* MIGRATE_MOVABLE list may include MIGRATE_RESERVEs */
			// page : 0x20000을 담당하는 struct page
			// zone : contig_page_data.node_zones[ZONE_NORMAL], 0, mt : 2
			__free_one_page(page, zone, 0, mt);
			// contig_page_data.node_zones[ZONE_NORMAL].free_area[0].free_list[MIGRATE_MOVABLE] 에
			// 0x20000 (pfn) struct page를 연결
			// contig_page_data.node_zones[ZONE_NORMAL].free_area[0].nr_free: 1
			...
			if (likely(!is_migrate_isolate_page(page))) {
				// zone : node_zones[ZONE_NORMAL], NR_FREE_PAGES : 0, 1
				__mod_zone_page_state(zone, NR_FREE_PAGES, 1);
				// contig_page_data.node_zones[ZONE_NORMAL].vm_stat[NR_FREE_PAGES] : 1
				// vm_stat[NR_FREE_PAGES] : 1
				// 두 전역변수에 페이지 수를 계속 추가해 줌

				if (is_migrate_cma(mt))	// 통과
					__mod_zone_page_state(zone, NR_FREE_CMA_PAGES, 1);
			}
		// to_free : 1, batch_free: 2
				...
				int migratetype)
{
	...
	// order 5 buddy를 contig_page_data에 매달음
	// contig_page_data.node_zones[ZONE_NORMAL].free_area[5].free_list[MIGRATE_MOVABLE] 에
	// 0x20000 (pfn) struct page를 연결
	// contig_page_data.node_zones[ZONE_NORMAL].free_area[5].nr_free: 1
	// order 5 인 free 상태의 buddy 갯수를 증가
...
void __init __free_pages_bootmem(struct page *page, unsigned int order)
{
	...
	// p : 0x20000을 담당하는 struct page의 주소
	prefetchw(p);
	// 캐시 공간에 page 공간을 올림
	...
	// order 5 buddy 가 free 상태인 것을 전역 변수들에게 설정해줌
	// 크기 정보와 어느 page가 order 5 buddy의 시작인지를 설정함
}
 ...
 * -- nyc
 */
// zone : node_zones[0], page : buddy에서 뽑아온 struct page 주소
// low : 0, high : 5, area : &node_zones[0].free_area[4].free_list[MIGRATE_MOVABLE]
// new_type : MIGRATE_UNMOVABLE
	...
	int migratetype)
{
	...
	while (high > low) {
		...
		// current order짜리를 쪼개서 order 0인 것 하나를 뽑는 작업을 이전에 수행함
		// 이제 남아있는 order 4, 3, 2, 1, 0인 페이지들을 다시 free_list에 등록시킴
	}
}
 ...
 */
// zone : node_zones[0], order : 0, migratetype : MIGRATE_UNMOVABLE
static inline
						...
						int migratetype)
{
	...
	for (current_order = order; current_order < MAX_ORDER; ++current_order) {
		...
		// 	  Buddy order 1인 페이지들이 연결되어 있는 곳임
 ...
 */
// zone : node_zones[0], start_page : 0x20000에 해당하는 struct page(가정)
// end_page : 0x20CFF에 해당하는 struct page(가정), migratetype : MIGRATE_UNMOVABLE
			  ...
			  int migratetype)
{
	...
	for (page = start_page; page <= end_page;) {
		...
		if (!PageBuddy(page)) {
			page++;
			continue;
			// 현재 page가 Buddy에 free로 등록되어 있지 않으면
			// 다음 page로 이동
		}
		// 여기까지 오면 현재 page가 Buddy에 free 상태로 등록되어 있는 것임
		order = page_order(page);
		// order를 가져옴

		list_move(&page->lru,
			  &zone->free_area[order].free_list[migratetype]);
		// free 상태인 페이지의 migratetype을 MIGRATE_MOVABLE에서
		// MIGRATE_UNMOVABLE로 이동시킴
		// free_list를 이동시켜면 migratetype이 바뀐 것임

		set_freepage_migratetype(page, migratetype);
		// page 구조체의 index를 migratetype으로 변경
		// 현재 page의 migratetype이 MIGRATE_MOVABLE로 변경된 것임
	...
	return pages_moved;
	// migratetype이 변경된 페이지의 개수가 반환됨
}

// zone : node_zones[0], page, migratetype : MIGRATE_UNMOVABLE
				...
				int migratetype)
{
	...
	// page가 속해있는 pageblock의 시작 페이지를 start_page로,
	// 마지막 페이지를 end_page로 저장
	// 즉 후보 migratetype에 존재하는 free page가 속해있는 pageblock을 구함

	/* Do not cross zone boundaries */
	if (!zone_spans_pfn(zone, start_pfn))
		// start_pfn이 zone 내부에 존재하는지 확인
		start_page = page;
	if (!zone_spans_pfn(zone, end_pfn))
		return 0;

	return move_freepages(zone, start_page, end_page, migratetype);
	// start_page에서 end_page까지의 페이지 중에 free 상태인 페이지를 찾아
	// migratetype을 MIGRATE_UNMOVABLE로 변경함
	// migratetype이 변경된 페이지의 개수가 반환됨
}
 ...
 * if it was unchanged).
 */
// zone : &node_zones, page : free_list에서 뽑아온 것
				  ...
				  int start_type, int fallback_type)
{
	 ...
	 */
	if (is_migrate_cma(fallback_type))	// 무조건 0
	...
	if (current_order >= pageblock_order / 2 ||
		...
		// zone : node_zones[0], page, start_type : MIGRATE_UNMOVABLE
		pages = move_freepages_block(zone, page, start_type);
		// page가 속해있는 pageblock의 페이지들 중 free 상태인 페이지들만
		// migratetype을 MIGRATE_UNMOVABLE로 변경함
		// 변경된 페이지 개수가 반환됨
		...
		if (pages >= (1 << (pageblock_order-1)) ||
			...
			// migratetype이 변경된 페이지 개수가 pageblock 내부 페이지 중
			// 절반 이상이 되면 pageblock의 migratetype을 변경함 
			return start_type;
		}
...
__rmqueue_fallback(struct zone *zone, int order, int start_migratetype)
{
	...
	for (current_order = MAX_ORDER-1; current_order >= order;
		...
		for (i = 0;; i++) {
			...
			// [1] migratetype : MIGRATE_RECLAIMABLE
			...
			// current_order : 10
			area = &(zone->free_area[current_order]);
			// area : &node_zones[0].free_area[10]

			if (list_empty(&area->free_list[migratetype]))
				// 모든 order에 대해 free_list[MIGRATE_RECLAIMABLE]은 항상 0이기 때문에
				// 무조건 continue로 이동함
				continue;
			// migratetype이 MIGRATE_MOVABLE로 설정되는 루프일 때 여기까지 도달 가능
			// 버디 할당자에 등록되어 있는 모든 페이지가 MIGRATE_MOVABLE에 등록되어 있기 때문임


			// area : &node_zones[0].free_area[4].free_list[MIGRATE_MOVABLE] 일 때로 가정
			page = list_entry(area->free_list[migratetype].next,
					struct page, lru);
			// free_list에 연결되어 있던 첫 번째 struct page의 시작 주소를 뽑아옴
			...
			new_type = try_to_steal_freepages(zone, page,
							  start_migratetype,
							  migratetype);
			// new_type : free_list에서 뽑아온 page가 속해있는
			// 	      pageblock의 migratetype이 저장됨
			// 그 외에, page가 속해있는 pageblock에 존재하는 모든 free page의
			// migratetype을 MIGRATE_MOVABLE에서 MIGRATE_UNMOVABLE로 변경함

			/* Remove the page from the freelists */
			list_del(&page->lru);
			// lru에 연결되어 있던 리스트를 제거
			// 결국 node_zones[0].free_area[4].free_list[MIGRATE_UNMOVABLE]에서 제거됨
			// free 상태가 아니게 됩니다

			rmv_page_order(page);
			// page의 private 멤버와 _mapcount 멤버를 초기화 해줌
			// _mapcount는 -1, private는 0으로 변경됨
			// 즉 buddy의 관리에서 빠져나오게 됨

			// zone : node_zones[0], page : buddy에서 뽑아온 struct page 주소
			// order : 0, current_order : 5, area : &node_zones[0].free_area[4].free_list[MIGRATE_MOVABLE]
			// new_type : MIGRATE_UNMOVABLE
			expand(zone, page, order, current_order, area,
			       new_type);
			// 이전에 current_order 크기의 페이지들을 buddy에서 받아 왔는데, 필요한 것은
			// order 0짜리 한 개임.
			// order 0 한 개를 뺀 나머지 공간을 다시 buddy에 등록시킴
			// 등록할 때, migratetype은 new_type으로 변경되어 재 등록되게 됨
...
retry_reserve:
	...
	// migratetype을 이용해 zone의 free list를 검색해 free 상태이면서
	// migratetype이 동일한 페이지를 찾아 그 page 구조체의 주소를 반환
	// MIGRATE_UNMOVABLE에 해당하는 free list에는 아무 것도 없으므로 NULL이 반환됨
	// 이 리스트는 버디 할당자에서 만들었음

	// migratetype : MIGRAGE_UNMOVABLE
	if (unlikely(!page) && migratetype != MIGRATE_RESERVE) {
		// zone : node_zones[0], order : 0, migratetype : MIGRATE_UNMOVABLE
		page = __rmqueue_fallback(zone, order, migratetype);
		// MIGRATE_MOVABLE에서 free 상태인 buddy 중 가장 큰 order를 갖는 것을 가져다
		// MIGRATE_UNMOVABLE로 변경하고, order 0인 페이지를 가져옴
		// 이 동작을 위한 buddy 변경 작업을 수행함
	...
	return page;
	// 찾으려는 migratetype에 맞고, order도 동일한 page를 반환함
}
 ...
 */
// zone : node_zones[0], order : 0, count : 1, list : pcp->lists
// migratetype : MIGRATE_UNMOVABLE, cold : 0
			...
			int migratetype, int cold)
{
	...
	for (i = 0; i < count; ++i) {
		...
		// node_zones[0]에서 migratetype과 order에 맞는 페이지를 가져옴
		// 이를 위해 buddy 구조와 page 내부 멤버 값을 전부 맞게 변경해줌
		...
		// 가져온 page를 node_zones[0]의 cpu0의 pcp에 연결
		// pcp list의 가장 앞에 가져다 붙임
	...
	// node_zones[0]의 cpu0의 pcp에 버디에서 가져온 order 0짜리 페이지를 연결

	// zone : node_zones[0], NR_FREE_PAGES : 30, -1
	__mod_zone_page_state(zone, NR_FREE_PAGES, -(i << order));
	// node_zones[0].vm_stat[NR_FREE_PAGES] 값에서 1을 빼서
	// 총 개수를 반영해줌
 ...
 */
// page : 0x20000을 담당하는 struct page, cold : 0
void free_hot_cold_page(struct page *page, int cold)
{
	...
	set_freepage_migratetype(page, migratetype);
	// page->index를 migratetype 값인 0x2로 저장함

	local_irq_save(flags);
	// IRQ 끄고 이전 설정 정보를 flags에 저장함

	__count_vm_event(PGFREE);
	// cpu0번의 vm_event_states.event[PGFREE] 값을 하나 증가시킴
	...
	// MIGRATE_PCPTYPES : 3, migratetype : 2
	if (migratetype >= MIGRATE_PCPTYPES) {
		if (unlikely(is_migrate_isolate(migratetype))) {
	...
	if (cold)
		...
		// cpu0의 &boot_pageset.pcp의 lists[2]에 현재 struct page를 연결
	pcp->count++;
	// cpu0의 &boot_pageset.pcp의 count 값을 증가
	// cpu0의 boot_pageset.pcp.count : 1, boot_pageset.pcp.high : 0
	if (pcp->count >= pcp->high) {
		unsigned long batch = ACCESS_ONCE(pcp->batch);
		// batch : 1

		// zone : node_zones[ZONE_NORMAL], batch : 1, pcp : boot_pageset.pcp
		...
		pcp->count -= batch;
		// cpu0의 boot_pageset.pcp.count : 0
	}
 ...
 * or two.
 */
// preferred_zone : 0, zone: contig_page_data->node_zones[0], order: 0,
// gfp_flags : __GFP_NOWARN | __GFP_NORETRY | __GFP_NOTRACK | __GFP_HARDWALL & !__GFP_NOFAIL
...
again:
	if (likely(order == 0)) {
		...
		local_irq_save(flags);
		pcp = &this_cpu_ptr(zone->pageset)->pcp;
		// node_zones[0]의 cpu0의 pcp가 저장됨

		list = &pcp->lists[migratetype];
		// list : pcp->lists[0]

		// 현재 pcp->lists[0]는 초기화만 되어 있는 상태임
		// list_empty(list) : 1
		if (list_empty(list)) {
			// pcp->count : 0, 
			// zone : node_zones[0], 0, pcp->batch : 1, list : pcp->lists
			// migratetype : MIGRATE_UNMOVABLE, cold : 0
			pcp->count += rmqueue_bulk(zone, 0,
					pcp->batch, list,
					migratetype, cold);
			// pcp->cout : 1로 증가
			// Buddy에서 migratetype이 일치하는 page를 가져오고,
			// 가져온 page를 node_zones[0]의 cpu0의 pcp에 연결
			// 이를 위한 자료들을 전부 반영해줌
		...
		if (cold)
			...
			// pcp의 리스트에 연결되어 있는 migratetype이 일치하는
			// 첫 번째 페이지를 반환
...
void __free_pages(struct page *page, unsigned int order)
{
	...
	if (put_page_testzero(page)) {
		if (order == 0)
			...
			// contig_page_data.node_zones[ZONE_NORMAL].vm_stat[NR_FREE_PAGES] : 32
			// vm_stat[NR_FREE_PAGES] : 32 로 저장함
			// struct page의 index 멤버에 migratetype(0x2)을 저장함
	}
}
...
static void __meminit zone_init_free_lists(struct zone *zone)
{
	int order, t;
	for_each_migratetype_order(order, t) {
	//for (order = 0; order < MAX_ORDER; order++) 		MAX_ORDER : 11
	//	for (t = 0; t < MIGRATE_TYPES; t++)		MIGRATE_TYPES : 4
...
static int __meminit zone_batchsize(struct zone *zone)
{
	...
	// batch에 해당하는 것은 zone의 전체의 1/1000 정도 되야하며
	// 512KB보다는 작아야 함.
...
int zone_wait_table_init(struct zone *zone, unsigned long zone_size_pages)
{
	...
	// zone->wait_table_hash_nr_entries : 0x400 (1024개)
	// 만들어 낼 hash의 pivot 칸 수를 구함
	zone->wait_table_bits =
		wait_table_bits(zone->wait_table_hash_nr_entries);
	// zone->wait_table_bits : 10
	// 1 << wait_table_bits 하면 hash의 pivot 칸 수를 뽑아낼 수 있음
	alloc_size = zone->wait_table_hash_nr_entries
					* sizeof(wait_queue_head_t);
	// alloc_size : hash의 pivot을 위한 크기를 계산하였음

	if (!slab_is_available()) {
		zone->wait_table = (wait_queue_head_t *)
			alloc_bootmem_node_nopanic(pgdat, alloc_size);
		// hash의 pivot을 할당 받고 그 위치의 주소값을 wait_table에 저장함
	} else {
		/*
	...
	// hash의 pivot 마다 달려있는 list를 초기화함
...
static __meminit void zone_pcp_init(struct zone *zone)
{
	...
	// 전역 변수인 boot_pageset의 시작 주소를 pageset 멤버에 저장
...
// zone, zone_start_pfn : 0x20000, size : 0x2F800, context : MEMMAP_EARLY
					...
					enum memmap_context context)
{
	...
	// zone->wait_table에 hash pivot을 만들어 달아줌.
	// 시작 주소가 wait_table 멤버에 저장됨
	if (ret)
		return ret;
	pgdat->nr_zones = zone_idx(zone) + 1;
	// nr_zones : 1
	// zone_idx(zone) : 현재 zone이 node_zones의 몇 번째 원소인지 반환
	...
	// zone->free_area[order].free_list[t] 를 전부 초기화 해 줌
	// order : 0 ~ 10, t : 0 ~ 3
	// 즉, 리스트 44개를 초기화 함
...
// pgdat : &contig_page_data, node_start_pfn : 0, node_end_pfn : 0
						...
						unsigned long *zholes_size)
{
	...
	for (i = 0; i < MAX_NR_ZONES; i++)
		totalpages += zone_spanned_pages_in_node(pgdat->node_id, i,
							 ...
							 node_end_pfn,
							 zones_size);
	// totalpages에 기존에 저장되어 있던 zones_size의 총 합을 계산해 저장함 
	pgdat->node_spanned_pages = totalpages;
	// 그 값을 node_spanned_pages 에 저장함
	...
	for (i = 0; i < MAX_NR_ZONES; i++)
		realtotalpages -=
			zone_absent_pages_in_node(pgdat->node_id, i,
						  node_start_pfn, node_end_pfn,
						  zholes_size);
	// realtotalpages 에는 hole의 크기를 빼서 실제 메모리의 4KB 페이지 갯수를 저장
						   ...
						   unsigned long present_pages)
{
	...
	if (spanned_pages > present_pages + (present_pages >> 4) &&
		...
		pages = present_pages;
	// hole이 특정 이상이면 present_pages가 유리하고
	// 특정 값 이하이면 그냥 spanned_pages 갯수로 그냥 유지하는 것이
	// 차후 메모리 관리 성능에 합당함.
		...
		unsigned long *zones_size, unsigned long *zholes_size)
{
	...
	// zone_start_pfn : 0x20000
	// contig_page_data에 저장되어 있던 값들을 다시 가져옴
	int ret;
	...
	// __init_waitqueue_head(&pgdat->kswapd_wait, "&pgdat->kswapd_wait", ??);
	// kswapd_wait 리스트를 초기화 하였음
	// list, spin_lock 초기화

	init_waitqueue_head(&pgdat->pfmemalloc_wait);
	// pfmemalloc_wait 리스트를 초기화 하였음
	// list, spin_lock 초기화
	...
	for (j = 0; j < MAX_NR_ZONES; j++) {
		...
		// zone_pgdat : &contig_page_data

		zone_pcp_init(zone);
		// 전역 변수인 boot_pageset의 시작 주소를 zone->pageset 멤버에 저장

		/* For bootup, initialized properly in watermark setup */
		mod_zone_page_state(zone, NR_ALLOC_BATCH, zone->managed_pages);

		lruvec_init(&zone->lruvec);
		// zone->lruvec.lists[0] ~ zone->lruvec.lists[4] 의 리스트를 전부 초기화
		if (!size)
			continue;
...
{
	/* Skip empty nodes */
	if (!pgdat->node_spanned_pages)	// 통과
		return;
		...
		unsigned long node_start_pfn, unsigned long *zholes_size)
{
	...
	// node_start_pfn : 0x20000
	init_zone_allows_reclaim(nid);
	// 통과
#ifdef CONFIG_HAVE_MEMBLOCK_NODE_MAP
	get_pfn_range_for_nid(nid, &start_pfn, &end_pfn);
#endif
	// pgdat : &contig_page_data, start_pfn : 0, end_pfn : 0
	// zones_size[0] : 0x2F800, zones_size[1] : 0x50800
	// zholes_size[0] : 0, zholes_size[1] : 0
	calculate_node_totalpages(pgdat, start_pfn, end_pfn,
				  zones_size, zholes_size);
	// pgdat->node_spanned_pages 에 총 4KB 페이지 갯수 저장
	// pgdat->node_present_pages 에 hole을 제외한 4KB 페이지 갯수 저장
	// 둘 다 0x80000
