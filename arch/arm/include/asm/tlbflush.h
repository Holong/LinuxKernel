...
static inline void local_flush_tlb_all(void)
{
	...
	if (tlb_flag(TLB_WB)) 	// 내부가 수행됨
		dsb();		// dsb 연산이 수행됨

	tlb_op(TLB_V4_U_FULL | TLB_V6_U_FULL, "c8, c7, 0", zero);
	tlb_op(TLB_V4_D_FULL | TLB_V6_D_FULL, "c8, c6, 0", zero);
	tlb_op(TLB_V4_I_FULL | TLB_V6_I_FULL, "c8, c5, 0", zero);
	tlb_op(TLB_V7_UIS_FULL, "c8, c3, 0", zero);			// tlb flush 수행됨
...
{
	const unsigned int __tlb_flag = __cpu_tlb_flags;
	// cpu_tlb 전역 변수에 저장되어 있는 flag 멤버의 값을 __tlb_flag에 저장함.
	// 이전에 ~~~ 에서 cpu_tlb 전역 변수를 0으로 초기화 하였으므로 현재 0 이 들어감.
	...
	//		f >> TBL_DCLEAN
	//		possible_tlb_flags >> TLB_DCLEAN | 어쩌구 저쩌구
	//		f >> TLB_DCLEAN
	...
	//		전부 풀면
	//
	//		tst 	__tlb_flag, TLB_DCLEAN
	//		mcrne	p15, 0, pmd, c7, c10, 1  	@ flush_pmd  실제로는 데이터 캐쉬 날림
	//
	//		위의 코드가 왜 들어감?
	//
	//		contex switch 마다 TLB를 날리려면 오버헤드가 큼
	//		ASID는 cache에 Process ID를 달아 cache에 해당하는 프로세스 까지 결정시키는 방법임.
	//		 8비트 값
	//		cache clean 중에 ASID를 가지고 날리는 방법이 하드웨어적으로 추가됨.
	//
	//		tlb를 지울때 tlb만 지우는 방법이 있고 tlb와 cache를 동시에 날리는 방법이 있음.
	//		
	//

	tlb_l2_op(TLB_L2CLEAN_FR, "c15, c9, 1  @ L2 flush_pmd", pmd);
	// if ~ else if 에서 전부 통과 됨.
	// 현재 타겟인 ARM Cortex A15는 TLB_L2CLEAN_FR 은 처리 되지 않음.
}
