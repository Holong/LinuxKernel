...
void __init exynos_init_io(void)
{
	...
	// 디바이스 트리를 스캔해서 매칭되는 아이디가 있으면 그 때 부름
	// chipid의 실제 주소 위치를 가상 주소 0xF8000000를 매핑한 뒤 이 내용을 vmlist와 static_vmlist에 등록하였음

	/* detect cpu id and rev. */
	// S5P_VA_CHIPID : 0xF8000000
	s5p_init_cpu(S5P_VA_CHIPID);
	// cpu id와 revision을 가져옴
...
static void __init exynos5_map_io(void)
{
	...
	// S5P_VA_SYSRAM
	// S5P_VA_CMU
	// S5P_VA_PMU 	을 실제 메모리 주소와 가상 메모리 주소를 매핑하고
	// 		vmlist와 static_vmlist에 등록시킴
